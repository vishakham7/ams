	...
	if request.method == "POST":
		...
		if my_form.is_valid():
			...
			if(row[0]==email and row[1]==password):
				# cursor.execute('SELECT session_id from masterApp_user where email=%s', [email])
				# row1 = cursor.fetchone()
				
				cursor.execute('UPDATE masterApp_user set session_id=""')
				request.session['email'] = email
				session_id = request.session.session_key
				# return HttpResponse(session_id)
				cursor.execute('UPDATE masterApp_user set session_id=%s where email = %s', [session_id, email])
...
def UserLogoutView(request):
	for sesskey in request.session.keys():
		cursor.execute('SELECT email from masterApp_user where session_id=%s', [session_id])
		row1 = cursor.fetchone()
		email = row1[0]
		return HttpResponse("ok")
		# cursor.execute('UPDATE masterApp_user set session_id="" where email=%s',[email])
